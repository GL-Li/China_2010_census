...
read_edu_data <- function(xlsFile) {
    ...
    colnames(eduMale) <- c("age", "population", "no_school", "elementary_school", 
                            "middle_school", "high_school", "technical_college", 
...
# education data for the nation, in city, town, and villiage
...
eduVilliage <- read_edu_data("xls_data/education_sect4_1c_villiage.xls")
eduVilliage$where <- "village"


# save RData for shiny ========================================================
save(eduNation, eduCity, eduTown, eduVilliage,
...
edu_combined <- rbind(eduCity, eduTown, eduVilliage)
g <- ggplot(edu_combined, aes(x = age, y = 100 * technical_college / population))
g + geom_line(aes(color=factor(sex, levels = c("male", "female")))) + 
    geom_point(aes(color=factor(sex, levels = c("male", "female")))) +
    facet_wrap(~where) +
    ggtitle("Compare Education in City, Town, and Villiage") +
    xlab("Age (years)") + 
